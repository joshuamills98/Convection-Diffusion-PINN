...
def numerical_solver(x, t, K, V):
    ...
    """
    Solves the convection diffusion equation numerically using a
    basic forward scheme.
    ...
    for ti in range(1, nt-1):
        for xi in range(1, nx-1):  # Domain
            T[xi, ti] = (1-2*K*dt/dx**2)*T[xi, ti-1] + \
                (K*dt/dx**2 + V*dt/(2*dx))*T[xi-1, ti-1] + \
                (K*dt/dx**2 - V*dt/(2*dx))*T[xi+1, ti-1]
...
def plot_contour(
    ...
    Arguments:
        ...
        V: Velocity Constant
        N_test_points: total number of domain poinnts over x
        and t will be N_test_points^2
    ...
    # Plot cross-sections to compare with numerical solution
    t_cross_sections = tslice

    # Demonstrate where cross-sections are being taken on main contour plot
    for cross_section in t_cross_sections:
    ...
    for i, t_cs in enumerate(tnew):
        ...
        plt.subplot(gs[1, i])
        txPe = np.stack([np.full(xnum.shape, t_cs/t_c),
                         xnum/x_c,
                         np.full(xnum.shape, Pe)], axis=-1)
        u = basemodel.predict(txPe, batch_size=nx*nt)
...
def ErrorAnalysisPlot(
    ...
    Plot the error between numeric solution and PINN
    over a specified range of K and V values
    ...
    # List over all the K and V and determine the error
    # between numerical scheme and PINN for each combination
    for i in range(n_scenarios**2):
        ...
        # Assert stability conditions for forward numerical solver
        assert(dx < 2*K/V)
        assert(dt < dx**2/(2*K))
        ...
        # Determine error between numerical scheme and PINN
        u = u.reshape(T.shape)
    ...
    plt.figure(figsize=(10, 8))  # Plot error map
