...
class Optimizer:
    ...
    def __init__(self, model, x_train, y_train, regularizer, maxiter, factr=1e5, m=50, maxls=50):

        self.model = model
        self.x_train = [ tf.constant(x, dtype=tf.float32) for x in x_train ]
        self.y_train = [ tf.constant(y, dtype=tf.float32) for y in y_train ]
        self.regularizer = regularizer
        # tf.print([x.shape for x in self.x_train])
        # tf.print([y.shape for y in self.y_train])
        self.shapes = [weights.shape for weights in self.model.get_weights()] #shapes of each weight layer
    ...
    def set_weights(self, flattened_weights):
        ...
        """
        Time taken by this method = 0.004 seconds to update weights, store in init?
        """
        index = 0
        new_weights = []
    ...
    def tf_evaluate(self, x,y):
        ...
        with tf.GradientTape() as g:
            ...
            f_out, c_ini, c_bound = self.model(x) #get outputs manually to feed into loss
            loss = tf.reduce_mean(tf.square(f_out-f_out_true))+self.regularizer*(tf.reduce_mean(tf.square(c_ini-c_ini_true)) + tf.reduce_mean(tf.square(c_bound-c_bound_true))) #determine three components of loss
    ...
    def fit(self, iterations = None, testing = False):
        ...
        print('Optimizer: L_BFGS Optimizer for {} iterations'.format(iterations))
        self.start = time.time()
        if testing:
            scipy.optimize.fmin_l_bfgs_b(func=self.evaluate, x0=initial_weights,m = self.m,
                factr=self.factr, maxls=50, maxiter=iterations, callback=self.testing_callback)  
        else:
            scipy.optimize.fmin_l_bfgs_b(func=self.evaluate, x0=initial_weights,m = self.m,
                factr=self.factr, maxls=50, maxiter=iterations, callback=self.callback)      
